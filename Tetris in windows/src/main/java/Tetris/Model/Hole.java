...
public class Hole {
    ...
    private int count;
    private FigureInHole figureInHole;      //проекция фигуры в колодце
    private int[] lines;
    private Figure figure;                  //текущая фигура
    private boolean gameOver;
    private Figure followingFigure;         //следующая фигура
    ...
    private void updateLines(){
        ...
        for (int j = 0; j< WORLD_HEIGHT; j++){            //удаление заполненных линий
            if (lines[j] == figureInHole.getBinarWidthHole()){
                try {
                Thread.sleep(300);                  //задержка при удалении линий
    ...
    //формирование показа следующей фигуры
    private List<BodyPart> getFollowingFigure() {
        ...
        for (int i=0;i < 4;i++){
            ...
            for (int j=0; j<b.length;j++){
                if (b[j] == '1')listFollowingFigure.add(new BodyPart(j + WORLD_WIDTH - 4, WORLD_HEIGHT - i -2,followingFigure.getColorFig()));
            }
        }
    ...
    //фигура падающая
    public List<BodyPart> getBodyFig() {

        List<BodyPart> bodyFig = new LinkedList<>();
        for (int i=2;i < WORLD_HEIGHT ;i++){
    ...
    //границы колодца и куча на дне
    public List<BodyPart> getBodyHole() {
        Color color = new Color(255,128,0);
        List<BodyPart> bodyHole = new LinkedList<>();
        for (int i=2;i < WORLD_HEIGHT ;i++){
